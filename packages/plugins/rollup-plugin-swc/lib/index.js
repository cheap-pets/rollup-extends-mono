...
export default function plugin (options = {}) {
  const { include, exclude, swcrc = true, browserslistrc = true } = options
  ...
  if (swcrc) {
    const swcrcPath = utils.isString(swcrc) ? swcrc : resolve(process.cwd(), '.swcrc')
  ...
  if (browserslistrc) {
    const targets = browserslist.loadConfig({
      path: utils.isString(browserslistrc) ? browserslistrc : process.cwd(),
      env: process.env.dev
    })
